...
int solution(string numbers) {
    ...
    int max = std::stoi(numbers);
    
    auto primes = prime_numbers(0, max);
    
    int answer = 0;
    
    for(const auto&e:primes)
    {
        unordered_multiset<char> temp(numbers.begin(), numbers.end());
        auto prime = to_string(e);
        auto all = all_of(prime.begin(), prime.end(), [&temp](const char& e){
            auto it = temp.find(e);
            if(it==temp.end())
            {
                return false;
